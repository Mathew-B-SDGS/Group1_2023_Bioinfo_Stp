...
This blueprint is used to create the database and add to the databas. 
all directories are relative to the app directory with /database/ added to the url
tables are Written using sqlalchemy ORM and the database is a sqlite database
...
db = SQLAlchemy(model_class=Base) # this is the database object
...
class TestType(db.Model):
    ...
    this table also stores a Json of the list of genes in the panel which can be accessed by the panel page
...
class TestCase(db.Model):
    ...
    this table also stores the date of the test and the scientist who ran the test
...
def sample_list():
    ...
    samples_list_db = db.session.execute(
        db.select(Patient).order_by(Patient.patient_id)).scalars() # forms a query object of all patients in the database
    return render_template("sample_list_all.html", sample=samples_list_db) # renders the template passing the query object
...
def sample_list_specific(sample_id):
    ...
    this is a dynamic route, the sample_id is passed in the url and used to query the database
    ...
    results = db.session.execute(stmnt_all).all() # execute the query and store the results
    # 3 lists are created to store the details of the 3 tables
    list_test_details1 = []
    list_test_details2 = []
    list_test_details3 = []
    # loop through the results and append the details to the lists (has to loop as the results are a list of tuples)
...
def sample_create():
    ...
    Url is dependent on the method, if the method is post then the sample is added to the database
    ...
    """
    if request.method == "POST": #http method is post and the form has been submitted
        sample = Sample( #instantiate a new sample object for the database
            sample_name=request.form["sample_name"].strip(), # access form and strip removes any white space from the input
            sample_type=request.form["sample_type"].strip()
        )
        db.session.add(sample)
        db.session.commit() # add and commit the new sample to the database
...
def panels_choice():
    ...
    depending on http method the form is rendered or the patient is added to the database

    input: html form with patient details
    output: html page with a form to create a new patient or a list of existing patients
    """
    if request.method == "POST": #http method is post and the form has been submitted
        patient_add = Patient( #instantiate a new patient object for the database
            patient_name=request.form["patient_name"].strip(),)
        db.session.add(patient_add)
        db.session.commit()
        some_patients = db.session.query(Patient.patient_name).all() # query the database for all patients
...
def update_patient_panel():
    ...
    this is a dynamic route, the patient_id is passed in the url and used to query the database

    input: session token with panel details  and patient_id (int) selected from the panel page
    output: dynamic F string with all details of the specific sample and session token
    """
    r_number = session['r']
    if request.method == "POST":

        object_add = TestCase(
            date_of_test=datetime.today().date(), # get todays date
            user=request.form["user"].strip(),
            which_patient=Patient.query.filter_by(
                patient_name=request.form["patient_selected"]).first(),
            which_testtype=TestType(
                testtype_name=session['panel_name'], #details are stored in the session token
